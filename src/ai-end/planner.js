...
function selectionPrompt() {
  return [
    ...
    '按用户填的出行人群、风格偏好、时间安排、「兴趣与要求」和「更多偏好」来取舍。',
    '如果 request.notes 包含地点范围约束（如「在酒店附近玩」「不要跑太远」「集中在某区域」），只选该范围内的 POI。',
...
function schedulePrompt() {
  return [
    ...
    '严格按 request.interests（兴趣与要求）和 request.notes（更多偏好）中的要求安排，例如用户注明想去的景点必须安排、某天中午要回酒店休息、只在酒店附近玩，则按要求执行。',
    '每天景点必须在地理上围绕酒店或同一区域集中，不得跨区来回跑。单日总里程不宜超过 80km，超出时应删减远处景点或调整到其他日期。',
    '只输出上述约定的完整行程 JSON。'
  ].join('\n');
}
...
function reviewPrompt() {
  return [
    ...
    '· 检查 request.notes 的范围约束是否被遵守（如只在酒店附近玩）；若单日里程明显过高（>80km）应调整。',
